...
// millis() ตอนพัดลมเพิ่งเปิด — global (เดิม static ใน readWind) ให้ status/cal อ่าน warm-up ได้
unsigned long fanOnTime = 0;
// ส่ง raw windMS stream ระหว่างเปิดหน้า calibration บนเว็บ (CAL_BEGIN/CAL_END คุม)
volatile bool g_calStreaming = false;
...
void readWind() {
    ...
    // fanOnTime = global (ดูประกาศใกล้ led1) — status/cal อ่าน warm-up ได้
    if (wasFanOff) {
...
void handle_mute_button() {
    ...
    if (currentBtnState == HIGH) {
        if ((millis() - pressStartTime >= 3000) && isLongPressHandled == false) {
            ...
            if (mode_senser == "ms") {
                mode_senser = "ft";
                saveSensorUnit(1);              // persist NVS
                Serial2.println("Mode: FT");
            } else {
                mode_senser = "ms";
                saveSensorUnit(0);              // persist NVS
...
void checkModeSensor() {
    ...
    if (btn == HIGH) {
        ...
        if (millis() - pressStart >= 2000 && !switched) {
            ...
            saveSensorUnit(mode_senser[0] == 'f' ? 1 : 0);   // persist NVS
            switched = true;
        }
    } else {
        pressStart = 0;
...
static void bleHandleCommand(const String& raw) {
    ...
    if (key == "ALERT") {                         // ตั้ง alert threshold (m/s) — setting, ไม่ต้องเข้า control mode
        float v = val.toFloat();
    ...
    if (key == "UNIT") {                          // หน่วยแสดงผล "ft" | "ms" — setting, persist NVS
    ...
    if (key == "CAL_BEGIN") { g_calStreaming = true;  Serial.println("[BLE] cal stream begin"); return; }
    if (key == "CAL_END")   { g_calStreaming = false; Serial.println("[BLE] cal stream end");   return; }
    ...
    else if (key == "CAL_ZERO") {                 // จุดศูนย์ (ไม่มีลม) — capture windMS ปัจจุบันเป็น rLow, ref=0
        if (sensorReady()) {
            saveLow(windMS, 0.00);
            Serial.printf("[BLE] CAL_ZERO raw=%.3f\n", windMS);
        } else Serial.println("[BLE] CAL_ZERO rejected (fan off / warm-up)");
    }
    else if (key == "CAL_HIGH") {                 // จุดสูง — capture windMS ปัจจุบันเป็น rHigh, ref = ค่าลมจริงที่กรอก
        float ref = val.toFloat();
        if (sensorReady() && ref > 0) {
            saveHigh(windMS, ref);
            Serial.printf("[BLE] CAL_HIGH raw=%.3f ref=%.2f\n", windMS, ref);
        } else Serial.println("[BLE] CAL_HIGH rejected (fan/warm-up/ref)");
    }
    else { Serial.printf("[BLE] unknown cmd: %s\n", cmd.c_str()); }
}

// sensor พร้อมอ่านค่าจริงหรือยัง = พัดลมเปิด + ผ่าน warm-up 5 วิ (ดู readWind gate)
// ใช้ทั้ง status JSON ("san") และ gate การ capture calibration
static bool sensorReady() {
    bool fanOn = objects.fan && lv_obj_has_state(objects.fan, LV_STATE_CHECKED);
    return fanOn && (millis() - fanOnTime >= 5000);
}
...
static String bleBuildStatus() {
    ...
    j += "\"san\":"     + String(sensorReady() ? 1 : 0);   // sensor warm-up พร้อม (gate cal)
    j += "}";
    return j;
}
...
void blePoll() {
    ...
    // raw windMS stream เฉพาะตอนเว็บเปิดหน้า calibration (CAL_BEGIN/CAL_END) — ~300ms
    // payload สั้น มี "raw" (ไม่มี "fan") → web decode() แยกออกจาก status JSON ได้
    static unsigned long lastCalRaw = 0;
    if (g_calStreaming && bleClientConnected && bleTxChar && millis() - lastCalRaw >= 300) {
        lastCalRaw = millis();
...
void setup()
{
    ...
    loadSensorUnit();                        // โหลดหน่วยแสดงผลเซ็นเซอร์ (ms/ft) จาก NVS
    mode_senser = (sensor_unit == 1) ? "ft" : "ms";
    Wire.begin(8, 9);
