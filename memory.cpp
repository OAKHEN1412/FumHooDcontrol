...
void saveSingleSlot(int dayIndex, int slotIndex, const ProgramData &data) {
    ...
    prefs.putInt((keyBase + "_res"   + s).c_str(), data.reserve[slotIndex]); // <--- [เพิ่ม] บันทึก reserve
...
ProgramData loadProgram(int index) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        data.reserve[i]= prefs.getInt((keyBase + "_res"   + s).c_str(), 0); // <--- [เพิ่ม] โหลด reserve
...
void createMockProgramData() {
    ...
    for (int day = 0; day < 7; day++) { // วนลูป 7 วัน (อาทิตย์ - เสาร์)
        ...
        for (int slot = 0; slot < 6; slot++) { // วนลูป 6 ช่วงเวลาต่อวัน
            ...
            mock.pump[slot]  = 0;
            mock.spray[slot] = 0;
            mock.reserve[slot] = 0; // <--- [เพิ่ม] กำหนดค่าเริ่มต้น reserve
